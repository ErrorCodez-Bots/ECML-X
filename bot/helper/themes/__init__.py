...
from bot.helper.themes import ecml_minimal

AVL_THEMES = {}
for theme in listdir("bot/helper/themes"):
    if theme.startswith("ecml_") and theme.endswith(".py"):
...
def BotTheme(var_name, **format_vars):
    ...
    if theme_ in AVL_THEMES:
        text = getattr(AVL_THEMES[theme_].ECMLStyle(), var_name, None)
        if text is None:
            LOGGER.error(
        ...
        text = getattr(rantheme.ECMLStyle(), var_name, None)

    if text is None:
        text = getattr(ecml_minimal.ECMLStyle(), var_name)
